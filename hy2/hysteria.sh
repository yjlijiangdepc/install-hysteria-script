...
inst_cert(){
    ...
    if [[ $certInput == 2 ]]; then
        ...
        openssl req -new -x509 -days 36500 -key /etc/hysteria/private.key -out /etc/hysteria/cert.crt -subj "/CN=www.sony.com.cn"
        chmod 777 /etc/hysteria/cert.crt
        chmod 777 /etc/hysteria/private.key
        hy_domain="www.sony.com.cn"
        domain="www.sony.com.cn"
    fi
}
...
inst_pwd(){
    read -p "设置 Hysteria 2 密码（直接回车随机密码）：" auth_pwd
...
changepasswd(){
    ...
    read -p "设置 Hysteria 2 密码（直接回车随机密码）：" passwd
    ...
    green "Hysteria 2 密码已成功修改为：$passwd"
    yellow "手动更新客户端配置文件"
    showconf
}
