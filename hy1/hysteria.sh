...
proxy-groups:
  - name: Proxy
    type: select
    proxies:
      - yjlijiangdepc-Hysteria
      
rules:
  - GEOIP,CN,DIRECT
  - MATCH,Proxy
EOF
    url="hysteria://$hy_ym:$port?protocol=$protocol&auth=$auth_pwd&peer=$domain&insecure=$true&upmbps=20&downmbps=100&alpn=h3#yjlijiangdepc-Hysteria"
    ...
    yellow "Hysteria 节点分享链接保存到 /root/hy/url.txt"
